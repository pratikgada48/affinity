...
import { pollCode } from "../constants";
import Results from "./Results";

const PollCoding = (props) => {
  ...
  return (
    <>
      <div className="poll-container">
        ...
        <div className="poll-question">{pollCode?.question}</div>
        <div className="poll-options">
          {pollCode.options.map((item, i) => (
            <div
              key={i}
        ...
        <button
          type="submit"
          className={`voteBtn ${vote ? " disabled" : " abled"}`}
          onClick={handleVote}
          disabled={vote}
        >
          Vote
